...
#filter out Deep C NO3 values from 11-07-2013
ChemWL_areas <- ChemWL_areas %>% filter(!(HPU == "deep C" & Date == mdy("11-07-2013") & name == "NO3 mg/L"))

Porosity <- 0.5

TotalArea_ha <- sum(HPUareas$Area_ha[HPUareas$HPU != "deep C"])
...
medians_by_HPUs <- concentrations_areas %>% 
  filter(HPU %in% c("E", "Bhs", "typical", "Bh", "bimodal", "deep C")) %>%
  group_by(name, HPU) %>%
...
sumplot <- concentrations_areas %>% 
  filter(HPU %in% c("E", "Bhs", "typical", "Bh", "bimodal", "deep C", "Watershed")) %>%
  filter(name == ForName) %>%
  mutate(dayofYear = yday(Date)) %>%
  ggplot(aes(dayofYear, conc_kgha, color = HPU))+
  geom_point()+
  theme_bw()+
  ylab(ForLabs)+
  facet_grid(rows = "HPU")
